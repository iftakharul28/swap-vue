<template>
  <section class="container my-9">
    ...
    <div className=" overflow-x-scroll hide-scrollbar -mx-4 mt-9" ref="scrollref">
      <div className="flex space-x-4 group" ref="cardref">
        <CategoryCard v-for="(list, i) in props.lists" :key="`card-no-${i}`" :title="list.accessibility.altText" :image="getFullImagePath(list.imageId, 'medium')" />
      </div>
    </div>
  </section>
</template>
